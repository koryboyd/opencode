...
app.use(
  ...
  createProxyMiddleware({
    ...
    onError: (err, req, res) => {
      ...
      res.status(503).json({ error: "Ollama unavailable", message: "Start Ollama with: ollama serve" })
    },
  }),
)
...
app.use(
  ...
  createProxyMiddleware({
    ...
    onError: (err, req, res) => {
      ...
      res.status(503).json({ error: "OpenCode server unavailable", message: "Start OpenCode server on port 3000" })
    },
  }),
)
