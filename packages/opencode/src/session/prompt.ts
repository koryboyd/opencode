...
NOTE: At any point in time through this workflow you should feel free to ask the user questions or clarifications. Don't make large assumptions about user intent. The goal is to present a well researched plan to the user, and tie any loose ends before implementation begins.
            ...
            const outcome: "break" | "continue" = yield* Effect.onExit(
              Effect.gen(function* () {
                ...
                const [skills, env, instructions, modelMsgs, memoryBlock, verificationBlock] = yield* Effect.all([
                  ...
                  skepticalMemory.systemPromptBlock(sessionID).pipe(Effect.orElseSucceed(() => "")),
                  // Verification obligations for unverified ephemeral claims
                  skepticalMemory.verificationObligation(sessionID).pipe(Effect.orElseSucceed(() => "")),
                ])
      ...
      const loop: (input: z.infer<typeof LoopInput>) => Effect.Effect<MessageV2.WithParts> = Effect.fn(
        ...
        return yield* runner.ensureRunning(runLoop(input.sessionID) as Effect.Effect<MessageV2.WithParts>)
      })
  ...
  const defaultLayer = Layer.unwrap(
    Effect.sync(() =>
      layer.pipe(
        ...
        Layer.provide(
          Layer.merge(SkepticalMemory.defaultLayer, AutoDream.defaultLayer),
        ),
      ),
    ),
  )
